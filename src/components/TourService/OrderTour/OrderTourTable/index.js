...
const OrderTourTable = ({ totalPrice, setTotalPrice }) => {
    ...
    listOrderTour.map((item) => {
        totalPricePerTour += item.price * item.quantity;
    }
    );

    setTotalPrice(totalPricePerTour);
    // console.log("totalPricePerTour", totalPrice);

    const handleDeleteTour = (row) => {
        alert("Xóa tour khỏi giỏ hàng");
        dispatch(removeOrder(row.idOrderTour))
